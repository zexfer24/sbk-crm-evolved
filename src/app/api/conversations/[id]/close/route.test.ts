...
/**
 * `close/route.ts` corre en dos capas de cliente Supabase (ver la cabecera
 * del route): la sesión normal para el UPDATE y el evento, y un cliente
 * admin solo para `record_handoff` (concedida SOLO a `service_role`, ver
 * 20260830040000). Este test fabrica los dos por separado, igual que
 * `messages/send/route.test.ts` fabrica el suyo para `fetchCurrentAgent`.
 */
...
/** Cada llamada a la RPC `record_handoff` del cliente admin, con sus parámetros. */
...
function fakeRequest(): Request {
  return new Request("http://crm.example/api/conversations/conv-1/close", { method: "POST" });
}
...
describe("POST /api/conversations/[id]/close", () => {
  ...
  it("cierra la conversación, deja el evento de sistema y el traspaso", async () => {
    ...
    expect(conversationUpdates).toContainEqual({ id: "conv-1", patch: { status: "closed" } });

    expect(insertedEvents[0]).toMatchObject({
      conversation_id: "conv-1",
      sender_type: "system",
      message_type: "system_event",
      content: "María cerró la conversación",
    });
