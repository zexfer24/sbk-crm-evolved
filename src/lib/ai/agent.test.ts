...
/**
 * T2, plan "Seis frentes del buzón" (8/9/2026): la reconfirmación antes de
 * pasar un caso a ventas vive DENTRO de `buildEscalateTool` (tools.ts), que
 * acá está completamente mockeado — `buildEscalateToolMock` no ejecuta la
 * máquina de estados real (esa la cubre `tools.test.ts` y
 * `handoff-confirmation.test.ts`). Lo que le toca probar a `agent.ts` es
 * otra cosa: que le pase los datos que la herramienta necesita, y que un
 * turno donde la herramienta decide "todavía no" (no muta `outcome`, se
 * queda en `{ escalated: false }`) termine exactamente como un turno normal
 * que nunca tocó la herramienta de escalar — texto entregado, sin la red de
 * seguridad de devolución/queja (que no aplica a intencion_compra) y con la
 * etapa limpia, no atascada en "tool_running".
 */
describe("runAgentTurn — reconfirmación de pase a ventas (T2, 8/9/2026)", () => {
  it("le pasa a buildEscalateTool el último mensaje del cliente y el sello de oferta pendiente", async () => {
    state.conversation = {
      ...state.conversation,
      handoff_confirmation_pending_at: "2026-09-08T10:00:00.000Z",
    };

    await runAgentTurn("conv-1");

    expect(buildEscalateToolMock).toHaveBeenCalledTimes(1);
    expect(buildEscalateToolMock.mock.calls[0][0]).toMatchObject({
      lastCustomerMessageAt: (state.conversation as { last_customer_message_at: string }).last_customer_message_at,
      handoffConfirmationPendingAt: "2026-09-08T10:00:00.000Z",
    });
  });

  it("sin ninguna oferta pendiente todavía (conversación nunca ofreció pasar a ventas), viaja null", async () => {
    state.conversation = { ...state.conversation, handoff_confirmation_pending_at: null };

    await runAgentTurn("conv-1");

    expect(buildEscalateToolMock.mock.calls[0][0]).toMatchObject({ handoffConfirmationPendingAt: null });
  });

  /**
   * Simula lo que hace la herramienta real (`buildEscalateTool`) cuando el
   * estado es "none"/"awaiting": el modelo la invocó, pero la herramienta NO
   * escala todavía — le pide al cliente que confirme, y `outcome` se queda
   * tal cual llegó (`{ escalated: false }`, el default de `runTurnPhases`).
   * El turno tiene que responder con el texto que redactó el modelo (la
   * reconfirmación) igual que si nunca hubiera pasado por la herramienta.
   */
  it("con la oferta de pase a ventas pendiente, el turno responde con texto y limpia la etapa (no escala)", async () => {
    generateMock.mockResolvedValueOnce({
      text: "Te paso con el equipo de ventas, ¿me confirmas con un sí?",
      usage: { inputTokens: 20, outputTokens: 8, totalTokens: 28 },
      steps: [{ toolCalls: [{ toolName: "escalarAAsesor" }] }],
    });
    ...
    expect(sendAgentTextMock).toHaveBeenCalledWith(
      expect.anything(),
      expect.anything(),
      "Te paso con el equipo de ventas, ¿me confirmas con un sí?",
      { isAutoReply: false }
    );
    expect(agentTurnInserts).toHaveLength(1);
