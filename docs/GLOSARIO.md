...
| `data.ts` | Capa de LECTURA de la bandeja/chat contra Supabase (55K; mapea filas crudas a tipos de dominio). `fetchConversations` gana `unreadOnly` y `assignedTo` (`FetchConversationsOptions`) para resolver contra la base las píldoras "No leídas" y "Mías" — antes "Mías" filtraba en memoria sobre la ventana cargada y un chat asignado viejo no aparecía. `InboxCounts` gana `unread` (mismo predicado que `unreadOnly`); `pending`/`pendingStale` siguen ahí para el panel de inicio, ya no tienen píldora propia en la bandeja |
...
| `inbox-filters.ts` | Las tres píldoras de la bandeja (No leídas/Mías/Todos), iguales para todos los roles; `DEFAULT_INBOX_FILTER = "unread"`. Exporta `isUnread` (LA definición de "sin leer": `unreadCount > 0 || manuallyUnread`, corte GLOBAL de equipo — cerrar un chat no es leerlo), `SERVER_FILTER_LIMIT` (tope de filas que piden a la base las píldoras que resuelve el servidor) y `serverFilterTruncated` (predicado puro: `rowCount === limit` y el contador exacto dice que hay más, para avisar ese recorte en vez de dejarlo mudo). La guardia anti-crecimiento (máx. 3 píldoras) vive en su test |
...
**inbox/:** `inbox-sidebar` (tres píldoras No leídas/Mías/Todos, abre en "No leídas"; secciones por píldora vía `inbox-sections` — ya no por ventana de 24h; "No leídas" y "Mías" se resuelven contra la base con `unreadOnly`/`assignedTo` de `data.ts`, `initialUnreadRows` siembra la primera para que abra con datos y no con "Buscando…"; contador de píldora desde la base; el buscador salta a "Todos"; cartel "Mostrando las 200 más recientes de N" —`.crm-list-truncated`— cuando `serverFilterTruncated` de `inbox-filters.ts` detecta que la consulta se topó con `SERVER_FILTER_LIMIT`), `conversation-list-item`, `filter-scroller`, `tag-filter-menu`, `conversation-context-menu`, `agent-home-panel` (panel de inicio: Pendientes / Esperando +24 h / Tuyas — la ventana de 24h que ya no vive en la bandeja sigue viva acá), `bcv-rate-chip`.
