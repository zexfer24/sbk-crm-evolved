...
**inbox/:** `inbox-sidebar` (tres píldoras No leídas/Mías/Todos, abre en "No leídas"; secciones por píldora vía `inbox-sections` — ya no por ventana de 24h; "No leídas" y "Mías" se resuelven contra la base con `unreadOnly`/`assignedTo` de `data.ts`, `initialUnreadRows` siembra la primera para que abra con datos y no con "Buscando…"; contador de píldora desde la base; el buscador salta a "Todos"), `conversation-list-item`, `filter-scroller`, `tag-filter-menu`, `conversation-context-menu`, `agent-home-panel` (panel de inicio: Pendientes / Esperando +24 h / Tuyas — la ventana de 24h que ya no vive en la bandeja sigue viva acá), `bcv-rate-chip`.
