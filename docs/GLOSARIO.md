...
**inbox/:** `inbox-sidebar` (tres píldoras No leídas/Mías/Todos, abre en "No leídas"; secciones por píldora vía `inbox-sections` — ya no por ventana de 24h; "No leídas" y "Mías" se resuelven contra la base con `unreadOnly`/`assignedTo` de `data.ts`, `initialUnreadRows` siembra la primera para que abra con datos y no con "Buscando…"; contador de píldora desde la base; el buscador salta a "Todos"; paginación por cursor (29/8/2026) también para estas dos píldoras — `serverRows` guarda `cursor`/`reachedEnd`/`loadingMore` propios y `loadMoreServerRows` pide la página siguiente con `cursorAfterPage` de lo acumulado, mismo `INBOX_PAGE_SIZE` y mismo botón "Cargar más" que ya tenía "Todos"; retira el tope mudo de 200 filas y su cartel de recorte), `conversation-list-item`, `filter-scroller`, `tag-filter-menu`, `conversation-context-menu`, `agent-home-panel` (panel de inicio: Pendientes / Esperando +24 h / Tuyas — la ventana de 24h que ya no vive en la bandeja sigue viva acá), `bcv-rate-chip`.
